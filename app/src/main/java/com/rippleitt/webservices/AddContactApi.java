...
{
    public void addContactApi(final Context context, final String contact_id )
    {
        final ArrayList<HashMap<String,String>> arry_Details=new ArrayList<>();

        final RequestQueue requestQueue = Volley.newRequestQueue(context);
        final StringRequest myRqst = new StringRequest(Request.Method.POST, RippleittAppInstance.BASE_URL +RippleittAppInstance.ADD_CONTACT, new Response.Listener<String>() {
            @Override
                  ...
                  //  maviLoaderHome.hide();
                    ...
                    if (status.equalsIgnoreCase("1")) {
                       // ((Activity)context).finish();
                        Toast.makeText(context,"Contact added successfully",TastyToast.LENGTH_SHORT).show();
                        ((Activity)context).finish();
                    }
                    else {
            ...
            protected Map<String, String> getParams() throws AuthFailureError {
               ...
               // params.put("token", RippleittAppInstance.TOKEN_ID);
                params.put("token", PreferenceHandler.readString(context,
                        PreferenceHandler.AUTH_TOKEN, "")); params.put("contact_id", contact_id);
